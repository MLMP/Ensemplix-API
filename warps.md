...
## ``` PUT /warp/create ``` 
Создание варпа. На счету игрока обязательно должно быть 50 ![alt text][ens]. Варп заработает после перезагрузки сервера.
Данный метод требует наличие OAuth авторизации.

[ens]: http://ensemplix.ru/images/ensi.png
### Пример запроса
...
| Параметры | Необходимость | Пояснение |
| --------- | ------------- | --------- |
| token     | Обязательно.  | Токен пользователя. |
| x         | Обязательно.  | Координата по оси X. |
| y         | Обязательно.  | Координата по оси Y. |
| z         | Обязательно.  | Координата по оси Z. |
| world     | Обязательно.  | Игровой мир. |
| greeting  | Опционально.  | Приветствие при телепоортации на варп. |
